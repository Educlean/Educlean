...
async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const userCredentials = await findOne<Account>("accounts", { employeeID }, { useCache: true, cacheTtl: 10 * 60 * 1000 })
        ...
        res.status(200).json({ message: "Login successful", role: userCredentials.role });
    } catch (error: unknown) {
