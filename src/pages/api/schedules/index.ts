...
interface ScheduleRow {
  employeeID: string;
  startDate: string | number; // puede ser string "YYYY-MM-DD" o número de Excel
  Mon?: string;
  Tue?: string;
  Wed?: string;
  Thu?: string;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      if (!uploaded) return res.status(400).json({ error: "No se recibió archivo" });
      ...
      for (const row of rows) {
        ...
        const user = await db.collection("users").findOne({ employeeID: employeeIDClean });
        if (!user) {
          console.warn(`No se encontró usuario con employeeId ${employeeIDClean}`);
          continue;
        }
